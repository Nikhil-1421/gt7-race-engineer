...
class GT7Communication(Thread):
    ...
    def stop(self):
        ...
        # A blocked recvfrom is NOT interrupted by closing the socket from
        # another thread (and the kernel keeps 33740 bound until the syscall
        # returns). Wake it with a self-addressed datagram instead; the run
        # loop checks _shall_run right after recvfrom and exits cleanly.
        try:
            w = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            w.sendto(b"\x00", ("127.0.0.1", self.receive_port))
            w.close()
        except OSError:
            pass
    def run(self):
        while self._shall_run:
            ...
            try:
                ...
                # Always release 33740 so a restart / runtime IP change can rebind.
                try:
                    if s is not None:
                        s.close()
                except OSError:
